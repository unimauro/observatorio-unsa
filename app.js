...
const GRANATE = '#0f8088', GRANATE2 = '#0d646d', ORO = '#dd9130', TEAL = '#b1461f';
...
const ink = () => isDark() ? '#9cbdbd' : '#5a6f70';
...
async function boot() {
  DATA.presu = await load('presupuesto-unsa.json');
  DATA.biblio = await load('bibliometria.json');
  DATA.prov = await load('proveedores-unsa.json');
  DATA.plan = await load('planilla-unsa.json');
  render();
}
...
function renderGasto() {
  ...
  if (d.por_programa?.length && typeof cProg !== 'undefined') {
    ...
    if (pn) pn.innerHTML = `La UNSA estructura su presupuesto por <strong>programa/actividad</strong>. Lo lidera «Ejercicio de la docencia universitaria» (${fmtM(p[0].pim)}) e «Investigación científica y tecnológica».`;
  }
  ...
  if (note) note.innerHTML = `Ejercicio ${d.anio} (cerrado). ` +
    ...
    `Fuente: MEF/SIAF, pliego 513 (UNSA).`;
}
...
function renderProv() {
  ...
  if (!V?.proveedores?.length) {
    w.innerHTML = `<div class="soon"><h3>🔎 En construcción</h3><p style="margin:0;color:var(--muted)">Descargando y cruzando las contrataciones de la UNSA (OECE/CONOSCE) para listar cada proveedor, su RUC, el monto y sus dueños/representantes.</p></div>`;
    return;
  }
  const t = V.totales || {};
  // KPIs
  ...
  w.innerHTML = `<div class="card">
    <h3>Proveedores de la UNSA · ${fmtN(V.proveedores.length)}</h3>
...
function renderInv() {
  ...
  const peers = [['uni', 'UNSA'], ['sanmarcos', 'San Marcos'], ['pucp', 'PUCP'], ['cayetano', 'Cayetano'], ['unalm', 'La Molina'], ['uni_ing', 'UNI']]
    .map(([k, n]) => DATA.biblio[k] ? { n, v: DATA.biblio[k].works } : null).filter(Boolean).sort((a, b) => b.v - a.v);
  new Chart(cPares, {
    type: 'bar', data: { labels: peers.map(x => x.n), datasets: [{ label: 'Publicaciones', data: peers.map(x => x.v), backgroundColor: peers.map(x => x.n === 'UNSA' ? GRANATE : ORO) }] },
    options: opts({ indexAxis: 'y', plugins: { legend: { display: false } } })
  });
  document.getElementById('invNote').textContent = `UNSA: ${fmtN(b.works)} publicaciones, ${fmtN(b.cited)} citas, h-index ${b.h_index} (OpenAlex, ${DATA.biblio._meta?.extraido || '2026'}). La principal universidad investigadora del sur del Perú.`;
}
...
function renderPlan() {
  ...
  if (!P?.personas?.length) {
    w.innerHTML = `<div class="soon"><h3>🔎 En construcción</h3><p style="margin:0;color:var(--muted)">Cruzando la planilla de la UNSA (docentes, funcionarios y personal, con sus remuneraciones) desde datos públicos del Estado (AIRHSP / portal de transparencia). Este portal <strong>solo muestra personal — nunca nombres de estudiantes</strong>.</p></div>`;
    return;
  }
...
function buildCtx() {
  ...
  let c = 'Eres el asistente del Observatorio UNSA, portal ciudadano de transparencia de la Universidad Nacional de San Agustín de Arequipa (UNSA, pliego 513), con datos públicos (MEF, OECE, OpenAlex). Responde corto, en español, solo sobre la UNSA y sus datos. No inventes cifras.';
